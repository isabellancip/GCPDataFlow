...
import com.hsbc.training.pipeline.function.ApplyFloorZeroAndReduceKeyFn;
import com.hsbc.training.pipeline.function.AverageOverScenarioFn;
import com.hsbc.training.pipeline.function.CollateralBalanceFn;
import com.hsbc.training.pipeline.function.LoggingFn;
...
import com.hsbc.training.pipeline.function.SumTradeResultFn;
...
public class NormalFlowPipeline {
	...
	public static void main(String[] args) {
		...
		PCollectionView<Map<String, LegalDoc>> legalDocMap = legalDoc
				.apply("Parse Line to LegalDoc Map", ParDo.of(new ParseLegalDocFn())).apply("View.asMap", View.asMap());
		...
		PCollection<KV<CompositeID, Map<String, double[]>>> collateralSet = resultTuple.get(collateral)
				.apply("Net Collateral", Combine.perKey(new SumTradeResultFn())).apply("Apply Collateral Balance",
						ParDo.of(new CollateralBalanceFn(legalDocMap)).withSideInputs(legalDocMap));
